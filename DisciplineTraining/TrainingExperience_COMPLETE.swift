...
final class AppState: ObservableObject {
    ...
    /// Returns discipline score trend for the last N days
    func disciplineTrend(lastDays: Int = 14) -> [DisciplineDataPoint] {
        let calendar = Calendar.current
        let today = calendar.startOfDay(for: Date())
        
        var dataPoints: [DisciplineDataPoint] = []
        
        // Generate data points for each day
        for dayOffset in (0..<lastDays).reversed() {
            guard let date = calendar.date(byAdding: .day, value: -dayOffset, to: today) else { continue }
            
            // Calculate score up to this date
            let weekBefore = calendar.date(byAdding: .day, value: -7, to: date) ?? date
            let relevantCheckIns = checkIns.filter { $0.date >= weekBefore && $0.date <= date }
            
            let score: Int
            if relevantCheckIns.isEmpty {
                score = 0
            } else {
                let completed = relevantCheckIns.filter { $0.completedTraining }.count
                let ratio = Double(completed) / Double(relevantCheckIns.count)
                score = Int((ratio * 100).rounded())
            }
            
            dataPoints.append(DisciplineDataPoint(date: date, score: score))
        }
